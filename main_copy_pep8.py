...
class BlogHandler(webapp2.RequestHandler):
    ...
    # Logout function used to logout user by deleting the user-id from the
    # stored cookie.
    def logout(self):
...
class PostPage(BlogHandler):
    ...
    def get(self, post_id):
        ...
        if self.user:
            liked = db.GqlQuery(
                "SELECT * FROM Like WHERE user = :1 AND post_id = :2", username, int(post_id)).get()
    ...
    def post(self, post_id):
        ...
        # User clicks unlike button
            ...
            if post and self.user:
                post.likes -= 1
                like = db.GqlQuery(
                    "SELECT * FROM Like WHERE post_id = :1 AND user = :2", int(post_id), self.user)
                key = like.get()
                key.delete()
                # key = db.Key.from_path('Post', int(post_id), parent=blog_key())
                # post = db.get(key)
...
class EditPostPage(BlogHandler):
    ...
    def post(self):
        ...
        # Finish the edit to the post if there is a subject and/or content.
...
class NewPost(BlogHandler):
    ...
    def post(self):
        ...
        # Otherwise, give them an error and ask to re-enter in the fields
        # again.
        if subject and content:
            p = Post(
                parent=blog_key(),
...
class Signup(BlogHandler):
    ...
    def post(self):
        ...
        if existing_user(self.username):
            params[
                'error_username'] = "That username is already exists, select a different one."
            have_error = True
...
class NewPassword(BlogHandler):
    ...
    def post(self):
        # Re-submit the user's credentials back in the Users DB and log them
        # in.
        self.username = self.request.get('username')
        self.password = self.request.get('password')
        self.verify = self.request.get('verify')
